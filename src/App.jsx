...
function App() {
  ...
  const generateReport = () => {
    ...
    if (zfHarnessLink) lines.push(`**ZF:** [Harness Link](${zfHarnessLink})`);
    if (caesarsLink) lines.push(`**Caesars:** [Harness Link](${caesarsLink})`);
    return lines.join("\n");
  };
    ...
    <div className="container">
      ...
      {commits.map((c, i) => (
        <div key={i} className="commit-row">
          <input
            value={c.message}
            onChange={(e) => updateCommit(i, "message", e.target.value)}
            placeholder="Commit message"
          />
          <input
            value={c.link}
            onChange={(e) => updateCommit(i, "link", e.target.value)}
            placeholder="Commit link"
          />
          {commits.length > 1 && (
            <button className="remove-btn" onClick={() => removeCommit(i)}>
              ✕
            </button>
          )}
        </div>
      ))}
      <button className="add-btn" onClick={addCommit}>
        + Add Commit
      </button>
      ...
      <input
        value={zfHarnessLink}
        onChange={(e) => setZfHarnessLink(e.target.value)}
        placeholder="https://..."
      />

      <label>Caesars Harness Link</label>
      <input
        value={caesarsLink}
        onChange={(e) => setCaesarsLink(e.target.value)}
        placeholder="https://..."
      />
